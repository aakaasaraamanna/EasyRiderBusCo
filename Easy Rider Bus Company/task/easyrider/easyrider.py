...
user_json = json.loads(input())
# user_json = json.loads(test_var2)

# bus_id_c, stop_id_c, stop_name_c, next_stop_c, stop_type_c, a_time_c = 0, 0, 0, 0, 0, 0

# for 1/5 part
# for bus in user_json:
#     if not re.match(r"\d{3}", str(bus['bus_id'])) or not isinstance(bus['bus_id'], int):
#         bus_id_c += 1
#
#     if not isinstance(bus['stop_id'], int):
#         stop_id_c += 1
#
#     # if not re.match(r"[A-Z].*( Road| Avenue| Boulevard| Street)$", str(bus['stop_name']).title()):
...
#         next_stop_c += 1
#
#     if not re.match(r"[SOF]?$", str(bus['stop_type'])):
#         stop_type_c += 1
#
#     if not re.match(r"(0[1-9]|1[0-9]|2[0-4]):(0[0-9]|[1-5][0-9]|60)$", str(bus['a_time'])):
#         a_time_c += 1
#
#
# total_error_c = bus_id_c + stop_id_c + stop_name_c + next_stop_c + stop_type_c + a_time_c
#
# print(f'Type and required field validation: {total_error_c} errors')
# print(f'bus_id: {bus_id_c}')
# print(f'stop_id: {stop_id_c}')
# print(f'stop_name: {stop_name_c}')
# print(f'next_stop: {next_stop_c}')
...
# for 2/5 part
# for bus in user_json:
#     if not re.match(r"[A-Z].*( Road| Avenue| Boulevard| Street)$", str(bus['stop_name'])):
#         stop_name_c += 1
#
#     if not re.match(r"[SOF]?$", str(bus['stop_type'])):
#         stop_type_c += 1
...
#     if not re.match(r"(0[1-9]|1[0-9]|2[0-4]):(0[0-9]|[1-5][0-9]|60)$", str(bus['a_time'])):
#         a_time_c += 1
# total_error_format_c = bus_id_c + stop_id_c + stop_name_c + next_stop_c + stop_type_c + a_time_c
# print(f'Format validation: {total_error_format_c} errors')
# print(f'stop_name: {stop_name_c}')
# print(f'stop_type: {stop_type_c}')
# print(f'a_time: {a_time_c}')
...
#         bus_data[bus_id] = 1
#     else:
#         bus_data[bus_id] = bus_data[bus_id] + 1
#
# print("Line names and number of stops:")
# for bus in bus_data:
#     print(f"bus_id: {bus}, stops:{bus_data[bus]}")
